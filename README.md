...
## Start a Project, footer, and legal pages

- Every "Start a Project" link opens the fast-contact panel (`#start-project`, a native `<dialog>` built by `startPanel()` in `build.mjs`): Instagram DM, text, email, and the Long Island booking form.
- The footer on every page carries the business name, location, contact email, and links to the Privacy Policy, Terms & Conditions, Cookie Policy (`cookies.html`), and Refund Policy (`refunds.html`). The last two are generated by `build.mjs` and are marked as drafts until the owner approves them.
- The public site sets no cookies and loads no analytics. The Spotify player is click-to-load, so Spotify is only contacted when a visitor asks for it. Fonts are self-hosted (the legal pages no longer call Google Fonts).
- The Long Island booking form requires a Privacy Policy / Terms checkbox. The payload sent to the Google Apps Script is unchanged.
- `assets/vendor/three/GLTFLoader.js` is patched to decode textures through an `<img>` instead of `createImageBitmap` + `fetch`. The original path dropped textures in Safari and under strict CSPs, which rendered the textured models white.
