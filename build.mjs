...
</head><body class="${className}" data-world="${world}" data-assets="${ROOT}assets/">
...
const img = (file, alt, w, h, attrs = 'loading="lazy"') => `<img src="${A}${file}" alt="${alt}" width="${w}" height="${h}" ${attrs}>`;

// ---------- Home ----------
const reel = [
  ['lifted/studio-microphone-lifted.webp', 'Microphone in the Long Island room', 'LI'],
  ['brooklyn-studio-wide.webp', 'Brooklyn room under the X ceiling installation', 'BK'],
  ['lifted/studio-guitar-lifted.webp', 'Guitar in red light', 'LI'],
  ['brooklyn-booth-window.webp', 'Brooklyn booth window and desk', 'BK'],
  ['lifted/studio-keys-lifted.webp', 'Keys lit blue in the studio', 'LI'],
  ['brooklyn-desk-detail.webp', 'Brooklyn desk and monitors', 'BK'],
  ['lifted/studio-console-lifted.webp', 'Hands on the console', 'LI'],
  ['brooklyn-gear-detail.webp', 'Outboard gear in Brooklyn', 'BK'],
  ['lifted/studio-astronaut-lifted.webp', 'Astronaut figure on the studio desk', 'LI'],
  ['brooklyn-x-daylight.webp', 'The Brooklyn X installation in daylight', 'BK'],
];
const reelItems = reel.map(([file, alt, tag]) => `<figure class="reel-item">${img(file, alt, 800, 560)}<figcaption>${tag === 'LI' ? 'Long Island' : 'Brooklyn'}</figcaption></figure>`).join('');
...
const home = page({
  ...
  body: `<section class="beat home-hero" data-beat="sky">
    ...
    <div class="wrap home-hero-grid">
      <div><p class="eyebrow"><span class="eyebrow-line"></span>One creative company / New York</p><h1 class="glitch-title">Music. Content.<br>Direction. <em>Growth.</em></h1></div>
      <div class="home-hero-side"><p class="hero-description">One creative company built to move artists, brands, and ideas forward.</p><div class="action-row"><a class="button button-light" href="#destinations">Explore PRXDIGY ${down}</a><a class="text-link" href="${instagram}" ${ext}>DM on Instagram ${arrow}</a></div></div>
    </div>
    <div class="scroll-cue" aria-hidden="true"><span></span>Scroll to descend</div>
  </section>
  <section class="beat home-clouds" data-beat="clouds">
    <div class="wrap"><p class="kicker" data-reveal>01 / 03</p><h2 class="mega" data-reveal="glitch">Make the work.<br><em>Move the work.</em></h2></div>
  </section>
  <section class="beat section destinations" id="destinations" data-beat="split"><div class="wrap"><div class="section-heading">${eyebrow('01 / Find your direction')}<h2 data-reveal="glitch">One name.<br><em>Three ways in.</em></h2><p data-reveal>Choose the space or team that fits what you're building.</p></div>
    <div class="destination-grid">
      <a class="destination" href="${L(destinations[0].href)}" data-reveal><div class="destination-image">${img('lifted/studio-sign-lifted.webp', 'PRXDIGY Studio sign inside the Long Island room', 1600, 1207)}</div><div class="destination-content"><span class="destination-index">01 / PRXDIGY Studio</span><span class="status">${destinations[0].status}</span><h3>Long Island</h3><p>Recording, mixing, production, content, and creative direction.</p><span class="destination-action">Enter Long Island ${arrow}</span></div></a>
      <a class="destination destination-creative" href="${L(destinations[1].href)}" data-reveal><div class="destination-image">${img('creative-projects-still.webp', 'PRXDIGY Creative Projects chrome and ruby ident', 1280, 854)}</div><div class="destination-content"><span class="destination-index">02 / PRXDIGY</span><span class="status status-neutral">Creative division</span><h3>Creative Projects</h3><p>Social media growth, content, campaigns, creative direction, and artist development.</p><span class="destination-action">Explore Creative Projects ${arrow}</span></div></a>
      <a class="destination" href="${L(destinations[2].href)}" data-reveal><div class="destination-image">${img('brooklyn-x-installation.webp', 'The glowing X cloud installation on the Brooklyn studio ceiling', 1800, 1199)}</div><div class="destination-content"><span class="destination-index">03 / PRXDIGY Studio</span><span class="status">${destinations[2].status}</span><h3>Brooklyn</h3><p>A new PRXDIGY studio location. Full location details are on the way.</p><span class="destination-action">Brooklyn Information ${arrow}</span></div></a>
    </div></div></section>
  <section class="reel" aria-label="Inside the PRXDIGY rooms"><div class="reel-track">${reelItems}${reelItems.replaceAll('<img ', '<img aria-hidden="true" ').replaceAll('<figure class="reel-item">', '<figure class="reel-item" aria-hidden="true">')}</div></section>
...
const gallery = [
  ['lifted/studio-sign-lifted.webp', 'PRXDIGY Studio emblem in the Long Island room', 1600, 1207, 'The room has its own point of view.', 'g-main'],
  ['lifted/studio-keys-lifted.webp', 'Keyboard used for production in the studio', 1600, 878, 'Built around the sound.', 'g-wide'],
  ['lifted/studio-guitar-lifted.webp', 'Guitar in the red-lit studio', 1600, 1033, 'Space to find the next idea.', ''],
  ['lifted/studio-microphone-lifted.webp', 'Vocal microphone with pop filter', 1600, 1090, '', ''],
  ['lifted/studio-console-lifted.webp', 'Hands on the keys in red light', 1600, 875, '', ''],
  ['lifted/studio-astronaut-lifted.webp', 'Astronaut figure on the desk', 1600, 838, '', ''],
  ['lifted/studio-blue-atmosphere-lifted.webp', 'Blue star ceiling over the room', 1600, 871, '', 'g-wide'],
];
...
const longIsland = page({
  ...
  body: `<section class="beat division-hero studio-hero" data-beat="hero"><div class="wrap division-hero-grid"><div class="division-hero-copy"><img class="division-logo" src="${A}studio-logo-alpha.webp" alt="PRXDIGY Studio" width="650" height="650"><p class="eyebrow"><span class="status">${destinations[0].status}</span> / Long Island</p><h1 class="glitch-title">The room where<br>the work <em>moves.</em></h1><p class="hero-description">Recording, mixing, production, content, and creative direction for artists ready to build.</p><div class="action-row"><a class="button button-light" href="#book">Apply to Book ${down}</a><a class="button button-outline" href="${sms}">Text PRXDIGY ${arrow}</a><a class="text-link" href="${instagram}" ${ext}>DM on Instagram ${arrow}</a></div></div></div><div class="hero-strip wrap" aria-hidden="true">${img('studio-mic.webp', '', 1800, 1226)}${img('lifted/studio-couch-lifted.webp', '', 1600, 961)}${img('lifted/studio-lounge-lifted.webp', '', 1600, 899)}</div></section>
  <section class="beat section listening-section" data-beat="listen"><div class="wrap listening-grid"><div>${eyebrow('01 / The sound')}<h2 data-reveal="glitch">Listen to<br><em>the work.</em></h2><p data-reveal>The records speak louder than the room.</p></div><div class="spotify-frame glass" data-reveal><iframe title="PRXDIGY Studio playlist on Spotify" src="https://open.spotify.com/embed/playlist/1gKUydEemsUPCpqJVEd2NQ?utm_source=generator&amp;theme=0" width="100%" height="352" loading="lazy" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" allowfullscreen></iframe></div></div></section>
  <section class="beat section room-gallery" data-beat="gallery"><div class="wrap"><div class="section-heading">${eyebrow('02 / Inside the room')}<h2 data-reveal="glitch">Where the records<br><em>get made.</em></h2><p data-reveal>Real details from the Long Island studio.</p></div><div class="gallery-grid">${gallery.map(([file, alt, w, h, cap, cls]) => `<figure class="${cls}" data-reveal>${img(file, alt, w, h)}${cap ? `<figcaption>${cap}</figcaption>` : ''}</figure>`).join('')}</div></div></section>
  <section class="beat section services-section" data-beat="services"><div class="wrap"><div class="section-heading">${eyebrow('03 / What we do')}<h2 data-reveal="glitch">Every part of<br><em>the record.</em></h2><p data-reveal>From the first idea to the final detail.</p></div><div class="service-list">${studioServices.map(([name, description], i) => `<div class="service-row" data-reveal><span>${String(i + 1).padStart(2, '0')}</span><h3>${name}</h3><p>${description}</p></div>`).join('')}</div></div></section>
  <section class="beat section room-story" data-beat="story"><div class="wrap story-grid"><div class="story-image" data-reveal>${img('lifted/studio-ceiling-lifted.webp', 'Atmospheric lighting across the studio ceiling', 1600, 871)}</div><div class="story-copy">${eyebrow('04 / The atmosphere')}<h2 data-reveal="glitch">The room is<br><em>everything.</em></h2><p data-reveal>The room sets the tone. The work finishes it. Starlight ceiling, red light, low glow—every detail is there to help you settle in and perform at your best.</p><div data-reveal>${img('studio-couch.webp', 'PRXDIGY Studio detail in red light', 1400, 841)}</div></div></div></section>
...
const creative = page({
  ...
  body: `<section class="beat division-hero creative-hero" data-beat="hero"><div class="wrap division-hero-grid"><div class="division-hero-copy"><img class="creative-hero-logo" src="${A}creative-logo.webp" alt="PRXDIGY Creative Projects" width="900" height="600"><p class="eyebrow">PRXDIGY / Creative Projects</p><h1 class="glitch-title">Creative direction<br>for projects that<br>need to <em>move.</em></h1><p class="hero-description">We build the strategy, content, music, visuals, and rollout that help artists and brands grow.</p><div class="action-row"><a class="button button-light" href="#creative-contact">Start a Creative Project ${down}</a><a class="text-link" href="${instagram}" ${ext}>DM on Instagram ${arrow}</a></div></div><div class="ident-frame"><video src="${A}creative-projects-ident.mp4" poster="${A}creative-projects-still.webp" autoplay muted loop playsinline preload="metadata" aria-label="PRXDIGY Creative Projects ident"></video><span class="image-caption">Concept / Production / Rollout</span></div></div></section>
  ...
  <section class="beat section selected-work" data-beat="work"><div class="wrap"><div class="section-heading">${eyebrow('03 / In practice')}<h2 data-reveal="glitch">Selected work</h2><p data-reveal>Anonymous glimpses into the production behind the work.</p></div><div class="work-grid"><figure class="work-item" data-reveal><div class="work-image">${img('creative-pill-chrome.webp', 'Chrome and glass PCP capsule with the ruby X, from the Creative Projects identity', 1254, 754)}</div><figcaption><span>01 / Artist rollout</span><h3>A visual world built around the release.</h3><p>Performance content, production, and rollout assets.</p></figcaption></figure><figure class="work-item" data-reveal><div class="work-image work-image-invert"><video src="${A}wordmark-glitch-reveal.mp4" poster="${A}wordmark-glitch-reveal-poster.jpg" autoplay muted loop playsinline preload="none" aria-label="PRXDIGY wordmark glitch reveal"></video></div><figcaption><span>02 / Visual production</span><h3>Make the moment feel like the music.</h3><p>Concept, shoot direction, editing, and social-ready content.</p></figcaption></figure></div><figure class="work-wide" data-reveal>${img('creative-pill-shatter.webp', 'The PCP capsule shattering on a wet black floor', 1672, 941)}</figure></div></section>
  <section class="beat section process-section" data-beat="process"><div class="wrap process-grid"><div class="process-heading">${eyebrow('04 / How it connects')}<h2 data-reveal="glitch">The full<br><em>package.</em></h2><p data-reveal>We can help shape the idea, define the direction, create the content, release the work, and measure what happens next.</p></div><div><ol class="process-list">${processSteps.map((step, i) => `<li data-reveal><span>${String(i + 1).padStart(2, '0')}</span>${step}</li>`).join('')}</ol><div class="support-list" data-reveal><p>Connected capabilities</p><ul>${support.map(item => `<li>${item}</li>`).join('')}</ul></div><p class="process-close" data-reveal>From the first idea to the public release.</p></div></div></section>
  <section class="beat section contact-section creative-contact" id="creative-contact" data-beat="contact"><div class="wrap contact-grid"><div class="panel">${eyebrow('05 / Make the next move')}<h2 data-reveal="glitch">Ready to build<br>something that <em>moves?</em></h2><p class="contact-lede" data-reveal>Tell us what you're building, and we'll map the next move.</p><a class="creative-start-link" href="#creative-contact-options">Start a Creative Project ${down}</a>${contactActions('Choose Instagram or text, and we’ll help map the next step.', 'creative-contact-options')}</div></div></section>`
});
...
const brooklyn = page({
  ...
  body: `<section class="beat division-hero brooklyn-hero" data-beat="hero"><div class="wrap division-hero-grid"><div class="division-hero-copy"><img class="division-logo" src="${A}studio-logo-alpha.webp" alt="PRXDIGY Studio" width="650" height="650"><p class="eyebrow"><span class="status">${destinations[2].status}</span> / Brooklyn</p><h1 class="glitch-title">Brooklyn is open.<br><em>The full experience</em><br>is being documented.</h1><p class="hero-description">For sessions, availability, and location information, reach out directly.</p><div class="action-row"><a class="button button-light" href="${instagram}" ${ext}>DM ${site.instagram.label}${arrow}</a><a class="button button-outline" href="${sms}">Text ${site.text.label}${arrow}</a></div><p class="brooklyn-note">Full location page in progress.</p></div></div></section>
  <section class="beat section bk-installation" data-beat="installation"><div class="wrap bk-install-grid"><figure class="bk-feature" data-reveal>${img('brooklyn-x-installation.webp', 'The glowing violet X cloud installation on the Brooklyn studio ceiling', 1800, 1199)}<figcaption>Brooklyn / The X</figcaption></figure></div></section>
  <section class="beat section bk-room" data-beat="room"><div class="wrap bk-grid">
    <figure class="bk-a" data-reveal>${img('brooklyn-studio-wide.webp', 'The Brooklyn room under the X ceiling', 1800, 1199)}</figure>
    <figure class="bk-b" data-reveal>${img('brooklyn-booth-window.webp', 'Desk and booth window in the Brooklyn studio', 1800, 2400)}</figure>
    <figure class="bk-c" data-reveal>${img('brooklyn-x-daylight.webp', 'The X installation and desk in daylight', 1179, 1542)}</figure>
    <figure class="bk-d" data-reveal>${img('brooklyn-desk-wide.webp', 'Brooklyn desk, couch, and monitors', 1179, 2556)}</figure>
  </div></section>
  <section class="beat section bk-detail" data-beat="detail"><div class="wrap bk-grid bk-grid-alt">
    <figure class="bk-a" data-reveal>${img('brooklyn-desk-detail.webp', 'Monitors and keyboard at the Brooklyn desk', 1800, 1199)}</figure>
    <figure class="bk-b" data-reveal>${img('brooklyn-hallway.webp', 'Hallway into the Brooklyn studio', 1179, 1535)}</figure>
    <figure class="bk-c" data-reveal>${img('brooklyn-gear-detail.webp', 'Outboard gear glowing in the dark', 1800, 1199)}</figure>
    <figure class="bk-d" data-reveal>${img('brooklyn-studio-wide-alt.webp', 'Wide view of the Brooklyn room in blue light', 1080, 720)}</figure>
  </div></section>
...
const files = PREVIEW ? [
  ['index.html', home], ['long-island.html', longIsland], ['creative-projects.html', creative], ['brooklyn.html', brooklyn], ['404.html', notFound],
] : [
  ['index.html', home],
